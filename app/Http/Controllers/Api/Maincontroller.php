...
class Maincontroller extends Controller
{
    ...
    public  function  UserBranches(){
        $data=DB::table('Users')
            ...
            ->select('users.Code', 'users.User_Name', 'sers.UserPassword', 'users.DefaultMandoobCode', 'users.DefaultCashBooksCode', 'users.DefaultBankCode', 'users.DefaultStoreCode')
            ->get();
...
where  ('IsDeleted', '=' ,0)->get();
    ...
    public  function  FItemD(){
        $data=DB::table('FItemD')->select('Code', 'Item_code', 'Item_Name',
            'Type1_Code', 'Sale_Price1', 'Sale_price2', 'Sale_price3', 'Max_Discount',
            'Category1', 'Category2', 'Category3', 'Add_Unit_Code', 'ItemGroupCode', 'DefaultItemTax',
            'TaxBreakCode', 'DefaultTableTaxPercent')
            ->where  ('IsDeleted', '=' ,0)
  ...
  public  function  ItemPriceList(){
    ...
    public  function  SalesProposalItems(){
        $data=DB::table('SalesProposal')->join('SalesProposalItems', 'SalesProposal.Code', '=', 'SalesProposalItems.ProposalCode')
            ...
            ->select('SalesProposalItems.ProposalCode', 'SalesProposal.ProposalStartDate', 'SalesProposal.ProposalEndDate',
                DB::raw('(select top 1 FItemD.Item_code from FItemD where SalesProposalItems.FItemD_Code = FItemD.Code) as Item_code'),
                'SalesProposalItems.Add_Unit_Details_Code', 'ProposalRule.ProposalQuantity', 'ProposalRule.Quantity', 'ProposalRule.DiscountPercent')
